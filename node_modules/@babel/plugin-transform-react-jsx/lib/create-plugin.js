...
You can set \`throwIfNamespace: false\` to bypass this warning.`);
    ...
    function convertAttribute(node) {
      ...
      if (_core.types.isJSXSpreadAttribute(node)) {
        return _core.types.spreadElement(node.argument);
      }
      ...
      return _core.types.inherits(_core.types.objectProperty(node.name, value), node);
    }
    ...
    function buildCreateElementOpeningElementAttributes(file, path, attribs) {
      if (RUNTIME_DEFAULT === "automatic" || get(file, "runtime") === "automatic") {
        const props = [];
        ...
        return props.length > 0 ? _core.types.objectExpression(props) : _core.types.nullLiteral();
      }
      ...
      for (const attr of attribs) {
        if (useSpread || !_core.types.isJSXSpreadAttribute(attr)) {
          props.push(convertAttribute(attr));
        } else {
          if (props.length) {
